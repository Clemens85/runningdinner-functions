...
def test_predict_3_team_clusters():
    ...
    original_clusters = original_routes['clusterNumber'].values

    clusterer = ClustererAgg(original_routes, data.get_distance_matrix(), data.get_cluster_sizes())
    result_routes, labels = clusterer.predict(n_clusters=3)
    assert len(labels) == 27
    assert set(labels) == {0, 1, 2}
    assert 'clusterNumber' in result_routes.columns

    assert all([a == b] for a, b in zip(result_routes['clusterNumber'].values, original_clusters))
