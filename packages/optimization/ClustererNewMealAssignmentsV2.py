...
class ClustererNewMealAssignmentsV2(Clusterer):
    ...
    def predict(self) -> Tuple[List[DinnerRoute], List[int]]:
        ...
        for cluster_template_idx, cluster_template in enumerate(self.cluster_templates):
            ...
            # Ensure route clusters are sorted with increasing cluster size starting with the smallest first
            route_clusters_sorted = self.__get_remaining_route_clusters_sorted_by_cluster_size(iterated_cluster_labels)
            if len(route_clusters_sorted) > 0:
                # Default case
                routes_of_cluster = route_clusters_sorted[0]
