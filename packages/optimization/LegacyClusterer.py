...
class LegacyClusterer(Clusterer):
    def __init__(self, data_provider: DataProvider):
        """
        Given DataProvider instance, initializes the LegacyClusterer with following attributes:
    ...
    def __remove_excess_meal_classes(self, routes_of_cluster: List[DinnerRoute], meal_label: str, excess: int) -> list:
        ...
        :param meal_label: The meal class to remove excess of
        :param excess: Number of excess meal classes to remove
        :return: Set of indices to remove
        """
        meal_class_indices = [route.originalIndex for route in routes_of_cluster if route.meal.label == meal_label]
    ...
    def __print_current_cluster_status(self, available_indices: set, cluster_labels_sorted: list):
        ...
        for cluster_label in cluster_labels_sorted:
            indices_of_cluster = [route.originalIndex for route in self.routes if route.clusterNumber == cluster_label]
            meal_labels_current_cluster = [route.meal.label for route in self.routes if
    ...
    def predict(self) -> Tuple[List[DinnerRoute], List[int]]:
        ...
        cluster_labels_sorted = sorted({route.clusterNumber for route in self.routes})
        meal_labels = sorted({route.meal.label for route in self.routes})
        ...
        for cluster_label in cluster_labels_sorted:
            ...
            for meal_label in meal_labels:
                ...
                if excess > 0:
                    Log.info(f"Cluster {cluster_label}: {meal_label} excess: {excess}")
                    removed_indices = self.__remove_excess_meal_classes(routes_of_cluster, meal_label, excess)
        ...
        for cluster_label in cluster_labels_sorted:
            ...
            for meal_label in meal_labels:
                ...
                if deficit > 0:
                    ...
                    # Find nearest available points of this meal
                    candidate_indices = [idx for idx in available_indices if self.routes[idx].meal.label == meal_label]
                    Log.info(f"Candidates for {meal_label} in cluster {cluster_label}: {candidate_indices}")
                    # For each needed, pick the closest to the current cluster
                    for _ in range(deficit):
                        if not candidate_indices:
                            raise ValueError(f"Not enough {meal_label} to fill cluster {cluster_label}")
                        ...
                        Log.info(
                            f"Chosen index for cluster {cluster_label}: {chosen_index} with distance {dists[min_idx]:.2f}")
